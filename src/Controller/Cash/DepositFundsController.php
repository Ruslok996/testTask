...
{
    #[Route('/api/deposit', name: 'deposit_funds', methods: ['POST'])]
    public function __invoke(Request $request, EntityManagerInterface $em): JsonResponse
    {
        ...
        if (!is_array($data) || !isset($data['accountId'], $data['amount'])) {
            return new JsonResponse(['message' => 'Invalid data.'], 400);
        }
        if (!is_numeric($data['amount'])) {
            return new JsonResponse(['message' => 'Invalid amount.'], 400);
        }
        $account = $em->getRepository(CashAccount::class)->find($data['accountId']);
        if (!$account) {
        ...
        return new JsonResponse(['message' => 'Deposit initiated.'], 201);
    }
}
